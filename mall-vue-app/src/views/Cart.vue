  ...
  <div class="cart-box">
      ...
      <van-checkbox-group @change="groupChange"
        ...
        <van-swipe-cell :right-width="50"
          ...
          <div class="good-item">
            ...
            <div class="good-img"><img :src="prefix(item.goodsCoverImg)"
                   alt=""></div>
            <div class="good-desc">
    ...
    <van-submit-bar v-if="list.length > 0"
      ...
      <van-checkbox @click="allCheck"
                    v-model="checkAll">全选</van-checkbox>
    </van-submit-bar>
    <!-- 缺省页 -->
    <div class="empty"
         v-if="!list.length">
      ...
      <van-button color="#1baeae"
                  type="primary"
                  @click="goTo"
                  block>前往首页</van-button>
    </div>
    <nav-bar></nav-bar>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    async deleteGood (id) {
      ...
      // 异步提交改变 mutations里面的state,dispatch数据提交至actions,可用于向后台提交数据
